...
export default function Gamepage() {
  ...
  const getGame = async () => {
    const response = await fetch(
      `https://api.rawg.io/api/games/${slug}?&key=3c2f5d3c0f3b464fbb39246b1aa12b59`
    );
  ...
  return (
    <article className="game-info">
      ...
      <aside>
        <img
          src={
            printgame.background_image === null
              ? "https://cdn.pixabay.com/photo/2016/11/03/14/18/stamp-1794352_960_720.png"
              : printgame.background_image
          }
          alt={"Picture from " + printgame.name}
        />
      </aside>
      ...
      <section className="game-info-lists">
        <ul>
          ...
          {printgame?.publishers?.map((p, index) => (
            <li key={index}>
              <b>Publisher</b>: {p?.name}
            </li>
          ))}
          <li>
            <b>Release date</b>: {printgame.released}
          </li>
          <li>
            <b>Website link</b>:
            <a href={printgame.website}>{printgame.website}</a>
          </li>
        </ul>
